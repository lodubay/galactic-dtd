...
\begin{deluxetable*}{Ccl}
    \tablecaption{A summary of parameters and their fiducial values for our multi-zone chemical evolution models. We omit parameters which have been left unchanged from \citet{Johnson2021-Migration}; see their Table 1 for details.\label{tab:multizone-parameters}}
    \tablehead{
        \colhead{Quantity} & \colhead{Value} & \colhead{Description}
    }
    \startdata
        t_D                 & 40 Myr                                & Minimum SN Ia delay time \\
        \Delta R_{\rm gal}  & Equation \ref{eq:radial-migration}    & Change in radius due to stellar migration (see Appendix \ref{app:migration}) \\
        z                   & Equation \ref{eq:sech-pdf}            & Distance from Galactic midplane at present day (see Appendix \ref{app:migration}) \\
        f_{\rm EB}(t|R_{\rm gal})   & Equation \ref{eq:earlyburst-ifr}  & Time-dependence of the infall rate in the early-burst model \\
        f_{\rm TI}(t|R_{\rm gal})   & Equation \ref{eq:twoinfall-ifr}   & Time-dependence of the infall rate in the two-infall model \\
...
The results of these tests are presented in Table \ref{tab:results}. Instead of reporting the numerical scores for each simulation output, in the column corresponding to a particular diagnostic we simply write \yes, \meh, or \no, which corresponds to a score in the top, middle, or bottom third out of all of our simulations, respectively. A \yes indicates that particular combination of model SFH and DTD performed better than most when compared to the APOGEE sample for that diagnostic.
