...
The uncertainties in the SN Ia DTD propagate into GCE models. In principle, the observed chemical abundance patterns should therefore contain information about the SN Ia DTD, and by extension their progenitors. A striking feature of the [O/Fe] distribution in the Milky Way is the distinct separation into two components, the high- and low-$\alpha$ sequences [references]. [Introduce other MW disk features].
...
[Talk about \citet{Matteucci2009-DTDModels} multi-zone models].
...
\begin{deluxetable*}{Cccl}
    ...
    \startdata
        ...
        p(\Delta R_{\rm gal}|\uptau,R_{\rm form}) & Equation \ref{eq:radial-migration}    & \ref{app:migration} & Probability density function of radial migration distance \\
        z                   & [-3, 3] kpc                & \ref{app:migration} & Distance from Galactic midplane at present day \\
        p(z|\uptau,R_{\rm final}) & Equation \ref{eq:sech-pdf}            & \ref{app:migration} & Probability density function of Galactic midplane distance\\
        ...
        \uptau_{\rm rise}             & 2 Gyr     & \ref{sec:sfh} & SFR rise timescale for inside-out and early-burst models \\
        ...
        \uptau_{\rm sfh}(R_{\rm gal}=8\,{\rm kpc})    & 15.1 Gyr  & \ref{sec:sfh} & SFH timescale at the solar annulus \\
    \enddata
\end{deluxetable*}
\vspace{-24pt}
...
\begin{table*}
    ...
    \begin{tabular}{lcll}
        ...
        Exponential & \ref{eq:exponential-dtd}  & $\uptau_{\rm Ia}=1.5$ Gyr    & \citet[][SD]{Greggio2005-AnalyticalRates};
                                                                      \citet{Schonrich2009-RadialMixing};       \\
                    &                           &                   & \citet{Weinberg2017-ChemicalEquilibrium}  \\
        Exponential & \ref{eq:exponential-dtd}  & $\uptau_{\rm Ia}=3$ Gyr   & --- \\
...
\begin{equation}
    f_{\rm Ia}^{\rm twopop}(t) = 0.5\Big[e^{-\frac{(t-t_p)^2}{2\sigma^2}} + e^{-t/\uptau}\Big]
    \label{eq:prompt-dtd}
\end{equation}
To approximate the DTD from \citet{Mannucci2006-TwoPopulations}, we take $t_p=50$ Myr, $\sigma=15$ Myr, and $\uptau_{\rm Ia}=3$ Gyr, which results in $\sim 50\%$ of SNe Ia exploding within $t<100$ Myr. As we illustrate in Figure \ref{fig:dtds}, the two-population DTD has a shorter median delay time than most other models (except the power-law with $\alpha=-1.4$, not shown).
...
\paragraph{Exponential} An exponentially declining DTD with timescale $\uptau_{\rm Ia}$:
\begin{equation}
    f_{\rm Ia}^{\rm exp}(t) = e^{-t/\uptau_{\rm Ia}}
    \label{eq:exponential-dtd}
\end{equation}
This model allows analytic solutions to the abundances as a function of time for some SFHs, making it a popular choice. \citet{Schonrich2009-RadialMixing} and \citet{Weinberg2017-ChemicalEquilibrium} both assume an exponential DTD with a timescale $\uptau_{\rm Ia}=1.5$ Gyr. We show in Appendix \ref{app:analytical-dtds} that a $\uptau_{\rm Ia}=1.5$ Gyr exponential is an adequate approximation for the analytical SD DTD from \citet{Greggio2005-AnalyticalRates}. 
...
In this paper, we investigate timescales $\uptau_{\rm Ia}=1.5$ and 3 Gyr.
...
\begin{equation}
    f_{\rm IO}(t|R_{\rm gal}) = (1 - e^{-t/\uptau_{\rm rise}}) e^{-t/\uptau_{\rm sfh}},
    \label{eq:insideout-sfh}
\end{equation}
where we assume $\uptau_{\rm rise}=2$ Gyr for all radii. The SFH timescale $\uptau_{\rm sfh}$ varies with $R_{\rm gal}$, with $\uptau_{\rm sfh}(R_{\rm gal}=8\,\rm{kpc})\approx15$ Gyr at the solar annulus and longer timescales in the outer Galaxy. The $\uptau_{\rm sfh} - R_{\rm gal}$ relation is based on the radial gradients in stellar age in Milky Way-like spirals measured by \citet[][see Section 2.5 of \citetalias{Johnson2021-Migration} for details]{Sanchez2020-StarFormationTimescales}.
...
Following \citetalias{Johnson2021-Migration}, we adopt $A_b=1.5$, $t_b=11.2$ Gyr, and $\sigma_b=1$ Gyr. The determination of $\uptau_{\rm sfh}$ 
...
\begin{equation}
    f_{\rm EB}(t|R_{\rm gal}) = e^{-t/\uptau_{\rm sfh}},
    \label{eq:earlyburst-ifr}
\end{equation}
where $\uptau_{\rm sfh}$ is the same as in the inside-out case.
To calculate $\dot \Sigma_\star$ from the above quantities, we modify the fiducial star formation law adopted from \citetalias{Johnson2021-Migration}, substituting $\uptau_{\rm EB}$ for the SFE timescale of molecular gas:
\begin{equation}
    \dot \Sigma_\star = 
    \begin{cases}
...
\begin{equation}
    ...
    f_{\rm TI}(t|R_{\rm gal}) = N_1(R_{\rm gal}) e^{-t/\uptau_1} + N_2(R_{\rm gal}) e^{-(t-t_{\rm on})/\uptau_2},
\end{equation}
...
%         % $\uptau_\star$        & 2.0 Gyr   & SFE timescale \\
...
%         % $\uptau_{\rm rise}$ & 2.0 Gyr     & SFR rise timescale \\
%         % $\uptau_{\rm sfh}$  & 15.1 Gyr  & SFR exponential \\
%         \hline
%     \end{tabular}
% \end{table}
...
We adopt the inside-out SFR (Equation \ref{eq:insideout-sfh}) evaluated at $R_{\rm gal}=8$ kpc (i.e., $\uptau_{\rm rise}=2$ Gyr and $\uptau_{\rm sfh}=15.1$ Gyr) and an SFE timescale $\uptau_\star\equiv M_g/\dot M_\star=2$ Gyr. Unless otherwise specified, we adopt an outflow mass-loading factor $\eta\equiv \dot M_{\rm out}/\dot M_\star=2.15$ \citepalias[see Equation 8 from][]{Johnson2021-Migration} and a minimum SN Ia delay time $t_D=40$ Myr. 
...
\begin{figure*}
    ...
    For reference, the solid gray curve represents an exponential DTD with $\uptau_{\rm Ia}=3$ Gyr. 
    \textit{Center:} Three models which assume an exponential DTD with varying timescale $\uptau_{\rm Ia}$. 
    % The solid, dashed, and dotted curves represent $\uptau=6$, 3, and 1.5 Gyr, respectively.
    ...
    All assume a post-plateau slope of $\alpha=-1.1$. For reference, the solid gray curve represents an exponential DTD with $\uptau_{\rm Ia}=3$ Gyr, and the dotted purple curve represents a power-law DTD with $\alpha=-1.1$ and no plateau.}
...
\begin{figure*}
    ...
    % minimum SN Ia delay times $t_D=40$ Myr (solid) and 150 Myr (dashed curves) for three DTDs: power-law with $\alpha=-1.1$ (blue), plateau with $W=0.3$ Gyr (green), and exponential with $\uptau_{\rm Ia}=1.5$ Gyr (purple curves). 
...
\begin{figure*}
    ...
    \textit{Left columns:} comparison between the inside-out and two-infall SFHs; both assume the exponential ($\uptau_{\rm Ia}=1.5$ Gyr) DTD. 
    \textit{Center column:} the distributions from APOGEE DR17 for reference, binned and smoothed similarly.
    \textit{Right columns:} comparison between the power-law ($\alpha=-1.4$) and exponential ($\uptau_{\rm Ia}=3$ Gyr) DTDs with the inside-out SFH. The MDFs in the inner Galaxy show the greatest change between the DTDs.}
    \label{fig:feh-df-comparison}
    \script{feh_df_comparison.py}
\end{figure*}

Figure \ref{fig:feh-df-comparison} shows MDFs across the Galaxy for a selection of models and APOGEE data. The two left-hand columns show MDFs from our models which assume the same DTD (exponential with $\uptau_{\rm Ia}=1.5$ Gyr) but different SFHs. For comparison, the APOGEE MDFs are binned similarly and presented in the center column of Figure \ref{fig:feh-df-comparison}.  The two right-hand columns

Holding the SFH fixed, varying the DTD has minimal influence on these MDFs for fixed SFH. The two right-hand columns of Figure \ref{fig:feh-df-comparison} plot the MDFs for two multi-zone models, which both assume an inside-out SFH but different DTDs: a power-law with slope $\alpha=-1.4$, and an exponential with timescale $\uptau_{\rm Ia}=3$ Gyr. The balance between prompt and delayed SNe Ia is starkly different between the two models, with $\sim 80\%$ of explosions occurring within 1 Gyr in the former but only $\sim 30\%$ in the latter. 
...
% \citet{Vincenzo2021-AlphaDistribution} demonstrate that the APOGEE selection function makes the observed $\alpha$ bimodality {\it weaker} in that the high-$\alpha$ sequence is undersampled relative to low-$\alpha$ stars.
...
Panels (f)--(i) in the bottom row of Figure \ref{fig:ofe-bimodality} illustrate the effect of the SFH on the [O/Fe] bimodality with the exponential ($\uptau_{\rm Ia}=1.5$ Gyr) DTD. The inside-out SFH does not produce a bimodal distribution for any of our DTDs. On the other hand, the early-burst SFH {\it always} produces a bimodal distribution in the $-0.4\leq{\rm [Fe/H]}<-0.2$ bin regardless of the assumed DTD, although the lower-metallicity bin does not (the lower peak falls short of our prominence threshold of 0.1). 
...
\begin{figure}
    ...
    \caption{A comparison of the [O/Fe]--[Fe/H] plane between the four SFHs in our multi-zone models. All assume the exponential ($\uptau_{\rm Ia}=1.5$ Gyr) DTD. Each panel plots a random mass-weighted sample of 10,000 star particles in the solar neighborhood ($7\leq R_{\rm gal}<9$ kpc, $0\leq|z|<0.5$ kpc) color-coded by $R_{\rm gal}$ at birth. A Gaussian scatter has been applied to all points based on the median abundance errors in APOGEE DR17 (see Table \ref{tab:sample}). The black curves represent the ISM abundance tracks in the 8 kpc zone. The red contours represent a 2-D kernel density estimate with a bandwidth of 0.03 of the APOGEE abundance distribution in that Galactic region, with the solid and dashed contours enclosing 30\% and 80\% of stars in the sample, respectively.}
...
\begin{equation}
    \Delta\uptau_{\rm RMS} \equiv \sqrt{\frac{\sum_k \Delta\uptau_k^2 n_{{\rm L23},k}}{n_{\rm L23,tot}}}
    \label{eq:age-ofe-score}
\end{equation}
where $\Delta\uptau_k=\rm{med}(\uptau_{\rm \vice})-\rm{med}(\uptau_{\rm L23})$ is the difference between the mass-weighted median star particle age in \vice and the median stellar age from \citetalias{Leung2023-Ages} in bin $k$, $n_{{\rm L23},k}$ is the number of stars from the \citetalias{Leung2023-Ages} age sample in bin $k$, and $n_{\rm L23,tot}$ is the total number of stars in the sample in that Galactic region. As before, the score for the model as a whole is the average of $\Delta \uptau_{\rm RMS}$ across all regions, weighted by the number of stars in the age sample in each region.
