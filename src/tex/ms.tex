...
to calculate galactocentric radius $R_{\rm gal}$ and midplane distance $z$, {\bf assuming a Sun--Galactic center distance $R_\odot=8.122$ kpc and height of the Sun above the midplane $z_\odot=20.8$ pc. Table \ref{tab:apogee-regions} lists the number of APOGEE stars in bins of $R_{\rm gal}$ and $|z|$. For some Galactic regions with $R_{\rm gal}<5$ kpc or $R_{\rm gal}>13$ kpc, the median distance error exceeds 1 kpc but remains within our bin width of 2 kpc, and the vast majority of stars have much smaller distance uncertainties.}
