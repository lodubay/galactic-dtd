...
\begin{figure*}
    ...
    \textit{Top middle:} A similar plot comparing exponential DTDs with varying timescales. The solid, dashed, and dotted curves represent timescales $\tau=6$, 3, and 1.5 Gyr, respectively.
    \textit{Top right:} Similar, comparing plateau DTDs with varying plateau widths. The solid, dashed, and dotted curves represent widths $W=1$, 0.3, and 0.1 Gyr, respectively. All plateau DTD models assume a post-plateau power-law slope of $\alpha=-1.1$. For reference, the dotted purple curve represents an exponential DTD with $\tau=3$ Gyr, and the dotted blue curve represents a power-law DTD $\alpha=-1.1$ and no plateau.
    \textit{Bottom left:} A similar plot comparing minimum SN Ia delay times of 40 Myr (solid) and 150 Myr (dashed) for two DTDs: a power-law with $\alpha=-1.1$ (blue), and an exponential with $\tau=1.5$ Gyr (purple).
    \textit{Bottom right:} A similar plot comparing all five functional forms for the DTD in this paper: triple-system (green), plateau with $W=1$ Gyr (cyan), exponential with $\tau=1.5$ Gyr (purple), power-law with $\alpha=-1.1$ (blue), and two-population (pink). In all cases a minimum SN Ia delay time of 40 Myr is assumed.}
    \label{fig:onezone-multipanel}
    \script{onezone_multipanel.py}
\end{figure*}
...
\begin{figure}
    ...
    \caption{Similar to Figure \ref{fig:age-ofe-dtd} but for simulations with different SFH models. All assume an exponential DTD with $\tau=1.5$ Gyr. In all panels, stars are limited to the Solar neighborhood ($7\leq R_{\rm gal}<9$ kpc, $0\leq|z|<0.5$ kpc)}
