...
\begin{equation}
    ...
    \begin{cases}
        t^{\alpha_1} (1 - A_{\rm{rise}} e^{-t/\tau_{\rm{rise}}}) & t \le \SI{1}{\giga\year} \\
        B t^{\alpha_2} (1 + A_{\rm{tail}} e^{-(t - \SI{1}{\giga\year})/\tau_{\rm{tail}}}) & t > \SI{1}{\giga\year}
    \end{cases}
...
\begin{equation}
    B = \frac{1}{1 + A_{\rm{tail}}} t^{(\alpha_1-\alpha_2)} (1 - A_{\rm{rise}} e^{-(\SI{1}{\giga\year})/\tau_{\rm{rise}}}).
\end{equation}
...
\begin{figure}
    \centering
    \includegraphics{figures/onezone_greggio05.pdf}
    \caption{Caption}
