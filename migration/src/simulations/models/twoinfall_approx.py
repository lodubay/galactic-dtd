...
class twoinfall_approx(double_exponential):

    def __init__(self, radius, dt = 0.01, dr = 0.1):
        super().__init__(onset = 4, ratio = 0.2) # dummy ratio value
        self.first.timescale = 2
        self.second.timescale = 2
        # self.first.timescale = 0.5 + radius / 15.5
        # self.second.timescale = 2 + 4 * (radius / 15.5)
        # self.ratio = self.amp_ratio(radius)
        # self.ratio = 0.5 * twoinfall_ampratio(self, gradient, radius,
        #     onset = self.onset, dt = dt, dr = dr)
        self.ratio = 5.6 * self.timescale_ratio()
